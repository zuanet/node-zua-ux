				...
				return html`
					<flow-expandable class="tx-row" static-icon expand ?txin=${tx.in} ?txmoved=${tx.isMoved} ?txout=${!tx.in}
						...
						<div class="tx-body">
							${tx.note}
							<div class="tx-id">
								<a target="_blank" href="https://explorer.zuacoin.com/txs/${tx.id}">${tx.id}</a>
							</div>
							<div class="tx-address">
								${tx.myAddress?T('COMPOUNDING WALLET => '):''}
								<a target="_blank" href="https://explorer.zuacoin.com/addresses/${tx.address}">${tx.address}</a>
							</div>
						</div>
					</flow-expandable>
				`
			...
			<div class="tx-list">
				${items.map((tx, i)=>{
					...
					<div class="tx-row" ?txin=${tx.in} tx-version=${tx.version} ?txmoved=${tx.isMoved} ?txout=${!tx.in}>
						...
						<div class="br tx-id">
							<a target="_blank" href="https://explorer.zuacoin.com/txs/${tx.id.split(":")[0]}">${tx.id.split(":")[0]}</a>
						</div>
						<div class="tx-address">
							${tx.myAddress?T('COMPOUNDING WALLET => '):''}
							<a target="_blank" href="https://explorer.zuacoin.com/addresses/${tx.address}">${tx.address}</a>
			...
			<div class="tx-list">
				${items.map((tx, i)=>{
					...
					<div class="tx-row" ?iscoinbase=${!tx.isCoinbase}>
						...
						<div class="br tx-id">
							<a target="_blank" href="https://explorer.zuacoin.com/txs/${tx.txId}">${tx.id}</a>
						</div>
						<div class="tx-address">
							<a target="_blank" href="https://explorer.zuacoin.com/addresses/${tx.address}">${tx.address}</a>
						</div>
