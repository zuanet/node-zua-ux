import {
	...
	baseUrl, ZUA, KAS, renderPagination, buildPagination, paginationStyle,
...
export class ZuaWalletMobile extends ZuaWalletUI{
	...
	render(){
		...
		let donationAddresses = [
			["Devfund donations:", "zua:qzd2ata4qcqwhstwz0az6n85skhsj4ed34uhaxjvx0g35yhtw796v4t7qzgca"],
			["Mining address:", "zua:qr5adhzernuagh6ea2scmaxz642cccnfcaqzpg4ej2wtrltx3g7p7vja9pdn9"],
		]
				...
				<div class="tab-content ${sCls('wallet')}" for="wallet">
					<div class="wallet-ux">
						...
						<flow-expandable class="donation-info" no-info no-icon icon="-">
							...
							<p is="i18n-p">
								if you wish to further the development of the zua ecosystem, we accept zua donations at the following addresses:
							</p>
							${
						...
						<flow-expandable class="developer-info" _expand no-info no-icon icon="-">
							...
							<div class="badge"><span is="i18n-span">Zua Core:</span> ${window.PWA_MODULES['@zua/core-lib']}</div>
							<div class="badge"><span is="i18n-span">Zua Wallet Framework:</span> ${window.PWA_MODULES['@zua/wallet']}</div>
							...
							<div class="badge"><span is="i18n-span">Zua UX:</span> ${window.PWA_MODULES['@zua/ux']}</div>
	...
	renderLockScreen(){
		return html`
			<div class="lock-screen">
				<div><img class="big-logo" src="/resources/images/zua.png"></div>
	...
	openAddressExplorer(address){
		if (!address)
			return
		let url = `http://explorer.zuanet.info/addresses/${address}`;
  			...
  			<div class="balance-badge">
				${ this.isLoading ? html`
					...
					<div class="balance pending">
						<span class="label-pending">${T('PLEASE WAIT')} <span class="dots">${this.dots}</span> ${total ? this.formatKAS(total)+' ZUA':''}</span>
					</div>
				` : html`
					<div class="balance">
						<span class="value">${this.formatKAS(available)} ZUA</span>
					</div>
					<div class="balance pending">
						<span class="label-pending" is="i18n-span">Pending:</span>
						<span class="value-pending">${this.formatKAS(pending)} ZUA</span>
					</div>
				`}
            </div>
